...
#include "P_css.h"
//-----------Народмон
bool postNmon = true;                               // включить/выключить публикацию своих датчиков на narodmon.ru
String macNmon;                                // уникальный идентификатор прибора в проекте
String nameNmon = "_Kuche";                         // название прибора в проекте (необязательное)
String ownerNmon;                              // владелец прибора в проекте (необязательное) логин или e-mail или мобильный номер владельца прибора в проекте для автоматической регистрации прибора
String latNmon = "";                                // широта, долгота, высота местоположения прибора
String lonNmon = "";
String altNmon = "";                                // высота над уровнем моря датчиков в метрах
byte periodSendNmon = 5;                            // период публикации на narodmon.ru в мин.
uint64_t timerNmon;
byte writeNmon[5] = {1, 2, 3, 0, 0};                // датчики для публикации на narodmon.ru
String buf;
//======================================================================
void setup(){
  Serial.begin(115200);
  ...
  macNmon.replace(":","");
  timerNmon=millis()+(periodSendNmon*60000);
  //----------OTA SETUP---------------------------------------------------
...
//                                  narodmon.ru
void narodmonSend(){
  if(WiFi.status()!=WL_CONNECTED||!postNmon) return;
  byte a=0;
  for(byte i=0;i<5;i++){
    if(writeNmon[i]!=0) a++;
  }
  if(a==0) return;
  buf="#VZ"+macNmon+"#VZ"+nameNmon+"\n";
  if(ownerNmon!="")buf+="#OWNER#"+ownerNmon+"\n";
  if(latNmon!=""&&lonNmon!=""){
    buf+="#LAT#"+latNmon+"\n";
    buf+="#LON#"+lonNmon+"\n"; 
  }
  if(altNmon!="")buf+="#ALT#"+altNmon+"\n";
  for(int i=0;i<5;i++){
    if(writeNmon[i]&&tempDallas[writeNmon[i]-1]!=85.0){
      buf+="#"+macNmon+String(i)+"#";
      buf+=String(tempDallas[writeNmon[i]-1])+"\n"; 
    }
  }
  buf+="##\r\n";
